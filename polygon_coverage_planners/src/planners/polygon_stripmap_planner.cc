...
bool PolygonStripmapPlanner::solve(const Point_2& start, const Point_2& goal,
                                   ...
                                   std::vector<std::vector<Point_2>>& clustered_solution) const {
  ...
  if (!runSolver(start_new, goal_new, solution, clustered_solution)) {
...
bool PolygonStripmapPlanner::runSolver(const Point_2& start,
  ...
  return sweep_plan_graph_.solve(start, goal, solution, clustered_solution);
}
