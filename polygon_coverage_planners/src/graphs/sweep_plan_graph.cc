...
bool SweepPlanGraph::solve(const Point_2& start, const Point_2& goal,
                           std::vector<Point_2>* waypoints,
                           std::vector<std::vector<Point_2>>& clustered_waypoints) const {
  ROS_ASSERT(waypoints);
  waypoints->clear();
  clustered_waypoints.clear();
  ...
  if (!temp_gtsp_graph.getWaypoints(solution, waypoints, clustered_waypoints)) {
...
bool SweepPlanGraph::getWaypoints(const Solution& solution,
  ...
  std::cout << "solution size is: " << solution.size() << std::endl;
  ...
  for (size_t i = 0; i < solution.size() - 1; ++i) {
    ...
    std::vector<Point_2> temp_waypoints;
    temp_waypoints.insert(temp_waypoints.end(), node_property->waypoints.begin(),
                      node_property->waypoints.end());
    clustered_waypoints.push_back(temp_waypoints);
